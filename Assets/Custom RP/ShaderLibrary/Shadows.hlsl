...
float FilterOtherShadow(float3 positionSTS)
{
	...
	float3 size = _ShadowAtlasSize.wwzz;
	OTHER_FILTER_SETUP(size, postionSTS.xy, weights, positions);
	float shadow = 0;
...
float GetCascadedShadow(DirectionalShadowData directional, ShadowData global, Surface surfaceWS)
{
	...
	float shadow = FilterDirectionalShadow(positionSTS);//SampleDirectionalShadowAtlas(positionSTS);
